...
class BotController extends Controller
{
            ...
            if ($input['type'] === 'contactRelationUpdate') {
                ...
                // regular chat message
                    ...
                    if ($message === 'help') {
                        ...
                        if ( ! $user = User::whereBotUserId($botUserId)->with('loginaccount')->first()) {
                            return SkypeResponse::message(trans('texts.not_authorized'));
                        }

                        Auth::onceUsingId($user->id);
                        $user->loginaccount->loadLocalizationSettings();
    ...
    private function authenticate($input)
    {
        ...
        $relationId = env('MSBOT_RELATION_ID');
        $relationSecret = env('MSBOT_RELATION_SECRET');
        $scope = 'https://graph.microsoft.com/.default';

        $data = sprintf('grant_type=relation_credentials&relation_id=%s&relation_secret=%s&scope=%s', $relationId, $relationSecret, $scope);
