...
class InvoiceController extends BaseController
{
    ...
    public function __construct(Mailer $mailer, InvoiceRepository $invoiceRepo, RelationRepository $relationRepo, InvoiceService $invoiceService, DocumentRepository $documentRepo, RecurringInvoiceService $recurringInvoiceService, PaymentService $paymentService)
    {
        // parent::__construct();
    ...
    public function edit(InvoiceRequest $request, $publicId, $clone = false)
    {
        ...
        $invoice = $request->entity()->load('invitations', 'loginaccount.country', 'relation.contacts', 'relation.country', 'invoice_items', 'documents', 'expenses', 'expenses.documents', 'payments');
        ...
        if ($clone) {
            ...
            $invoice->invoice_number = $company->getNextInvoiceNumber($invoice);
        ...
        if ($entityType == ENTITY_QUOTE) {
            ...
            foreach ($invoice->payments as $payment) {
                ...
                if (count($invoice->payments) > 1) {
                    $label .= ' - ' . $company->formatMoney($payment->amount, $invoice->relation);
                }
    ...
    private static function getViewModel($invoice)
    {
        ...
        $recurringDueDates = [
            trans('texts.use_relation_terms') => ['value' => '', 'class' => 'monthly weekly'],
        ];
        ...
        return [
            ...
            'invoiceLabels' => Auth::user()->loginaccount->getInvoiceLabels(),
    ...
    private function emailRecurringInvoice(&$invoice)
    {
        if (!$invoice->shouldSendToday()) {
            if ($date = $invoice->getNextSendDate()) {
                $date = $invoice->loginaccount->formatDate($date);
                $date .= ' ' . DEFAULT_SEND_RECURRING_HOUR . ':00 am ' . $invoice->loginaccount->getTimezone();
    ...
    public function invoiceHistory(InvoiceRequest $request)
    {
        ...
        $invoice->load('user', 'invoice_items', 'documents', 'expenses', 'expenses.documents', 'loginaccount.country', 'relation.contacts', 'relation.country');
        $invoice->invoice_date = Utils::fromSqlDate($invoice->invoice_date);
        $invoice->due_date = Utils::fromSqlDate($invoice->due_date);
        $invoice->features = [
            'customize_invoice_design' => Auth::user()->hasFeature(FEATURE_CUSTOMIZE_INVOICE_DESIGN),
        ...
        foreach ($activities as $activity) {
            ...
            $backup->loginaccount = $invoice->loginaccount->toArray();
