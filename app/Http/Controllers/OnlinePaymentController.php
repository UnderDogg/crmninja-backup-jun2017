...
class OnlinePaymentController extends BaseController
{
    ...
    public function showPayment($invitationKey, $gatewayType = false, $sourceId = false)
    {
        ...
        $invitation = $invitation->load('invoice.relation.loginaccount.account_gateways.gateway');

        if ( ! $gatewayType) {
            $gatewayType = Session::get($invitation->id . 'gateway_type');
        }

        $paymentDriver = $invitation->loginaccount->paymentDriver($invitation, $gatewayType);
    ...
    public function offsitePayment($invitationKey = false, $gatewayType = false)
    {
        ...
        $invitation = Invitation::with('invoice.invoice_items', 'invoice.relation.currency', 'invoice.relation.loginaccount.account_gateways.gateway')
    ...
    public function handlePaymentWebhook($companyKey, $gatewayId)
    {
        ...
        if (!$company) {
            return response()->json([
                'message' => 'Unknown loginaccount',
            ], 404);
        }

        $accountGateway = $company->getGatewayConfig(intval($gatewayId));
    ...
    public function handleBuyNow(RelationRepository $relationRepo, InvoiceService $invoiceService, $gatewayType = false)
    {
        ...
        if ( ! $company || ! $company->enable_buy_now_buttons || ! $company->hasFeature(FEATURE_BUY_NOW_BUTTONS)) {
            return redirect()->to("{$redirectUrl}/?error=invalid loginaccount");
        }
