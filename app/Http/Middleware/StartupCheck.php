...
class StartupCheck
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        // Check if we're requesting to change the loginaccount's language
        if (Input::has('lang')) {
            ...
            $locale = Auth::user()->loginaccount->language ? Auth::user()->loginaccount->language->locale : DEFAULT_LOCALE;
            App::setLocale($locale);
        ...
        // Make sure the loginaccount/user localization settings are in the session
            ...
            if (!$claimingLicense && Input::has('license_key') && Input::has('product_id')) {
                ...
                if ($productId == PRODUCT_INVOICE_DESIGNS) {
                    ...
                    if ($data && $data != RESULT_FAILURE) {
                        $corporation = Auth::user()->loginaccount->corporation;
                        $corporation->plan_term = PLAN_TERM_YEARLY;
                        $corporation->plan_paid = $data;
                        $corporation->plan_expires = date_create($data)->modify('+1 year')->format('Y-m-d');
