...
Route::post('/get_started', 'LoginAccountController@getStarted');

// Relation visible pages
Route::group(['middleware' => 'auth:relation'], function () {
    Route::get('view/{invitation_key}', 'RelationPortalController@view');
    Route::get('download/{invitation_key}', 'RelationPortalController@download');
    ...
    Route::get('relation/payment_methods', 'RelationPortalController@paymentMethods');
    Route::post('relation/payment_methods/verify', 'RelationPortalController@verifyPaymentMethod');
    //Route::get('relation/payment_methods/add/{gateway_type}/{source_id?}', 'RelationPortalController@addPaymentMethod');
    //Route::post('relation/payment_methods/add/{gateway_type}', 'RelationPortalController@postAddPaymentMethod');
    Route::post('relation/payment_methods/default', 'RelationPortalController@setDefaultPaymentMethod');
    Route::post('relation/payment_methods/{source_id}/remove', 'RelationPortalController@removePaymentMethod');
    Route::get('relation/quotes', 'RelationPortalController@quoteIndex');
    Route::get('relation/invoices', 'RelationPortalController@invoiceIndex');
    Route::get('relation/invoices/recurring', 'RelationPortalController@recurringInvoiceIndex');
    Route::post('relation/invoices/auto_bill', 'RelationPortalController@setAutoBill');
    Route::get('relation/documents', 'RelationPortalController@documentIndex');
    Route::get('relation/payments', 'RelationPortalController@paymentIndex');
    Route::get('relation/dashboard', 'RelationPortalController@dashboard');
    Route::get('relation/dashboard/{contact_key}', 'RelationPortalController@contactIndex');
    Route::get('relation/documents/js/{documents}/{filename}', 'RelationPortalController@getDocumentVFSJS');
    Route::get('relation/documents/{invitation_key}/{documents}/{filename?}', 'RelationPortalController@getDocument');
    Route::get('relation/documents/{invitation_key}/{filename?}', 'RelationPortalController@getInvoiceDocumentsZip');

    Route::get('api/relation.quotes', ['as' => 'api.relation.quotes', 'uses' => 'RelationPortalController@quoteDatatable']);
    Route::get('api/relation.invoices', ['as' => 'api.relation.invoices', 'uses' => 'RelationPortalController@invoiceDatatable']);
    Route::get('api/relation.recurring_invoices', ['as' => 'api.relation.recurring_invoices', 'uses' => 'RelationPortalController@recurringInvoiceDatatable']);
    Route::get('api/relation.documents', ['as' => 'api.relation.documents', 'uses' => 'RelationPortalController@documentDatatable']);
    Route::get('api/relation.payments', ['as' => 'api.relation.payments', 'uses' => 'RelationPortalController@paymentDatatable']);
    Route::get('api/relation.activity', ['as' => 'api.relation.activity', 'uses' => 'RelationPortalController@activityDatatable']);
});
...
Route::post('signup/validate', 'LoginAccountController@checkEmail');
Route::post('signup/submit', 'LoginAccountController@submitSignup');
...
if (Utils::isNinja()) {
    Route::post('/signup/register', 'LoginAccountController@doRegister');
    Route::get('/news_feed/{user_type}/{version}/', 'HomeController@newsFeed');
    Route::get('/demo', 'LoginAccountController@demo');
}
...
Route::group(['middleware' => 'auth:user'], function () {
    ...
    Route::get('view_archive/{entity_type}/{visible}', 'LoginAccountController@setTrashVisible');
    Route::get('hide_message', 'HomeController@hideMessage');
    Route::get('force_inline_pdf', 'UserController@forcePDFJS');
    Route::get('loginaccount/get_search_data', ['as' => 'get_search_data', 'uses' => 'LoginAccountController@getSearchData']);
    Route::get('check_invoice_number/{invoice_number}', 'InvoiceController@checkInvoiceNumber');

    Route::get('settings/user_details', 'LoginAccountController@showUserDetails');
    Route::post('settings/user_details', 'LoginAccountController@saveUserDetails');
    Route::post('users/change_password', 'UserController@changePassword');

    Route::resource('relations', 'RelationController');
    Route::get('api/relations', ['as' => 'api.relations', 'uses' => 'RelationController@getDatatable']);
    Route::get('api/activities/{relation_id?}', ['as' => 'api.activities', 'uses' => 'ActivityController@getDatatable']);
    Route::post('relations/bulk', 'RelationController@bulk');

    Route::resource('tasks', 'TaskController');
    Route::get('api/tasks/{relation_id?}', ['as' => 'api.tasks', 'uses' => 'TaskController@getDatatable']);
    Route::get('tasks/create/{relation_id?}', 'TaskController@create');
    Route::post('tasks/bulk', 'TaskController@bulk');

    Route::get('api/recurring_invoices/{relation_id?}', ['as' => 'api.recurring_invoices', 'uses' => 'InvoiceController@getRecurringDatatable']);

    Route::get('invoices/invoice_history/{invoice_id}', 'InvoiceController@invoiceHistory');
    Route::get('quotes/quote_history/{invoice_id}', 'InvoiceController@invoiceHistory');

    Route::resource('invoices', 'InvoiceController');
    Route::get('api/invoices/{relation_id?}', ['as' => 'api.invoices', 'uses' => 'InvoiceController@getDatatable']);
    ...
    Route::get('api/quotes/{relation_id?}', ['as' => 'api.quotes', 'uses' => 'QuoteController@getDatatable']);
    Route::post('quotes/bulk', 'QuoteController@bulk');

    Route::resource('payments', 'PaymentController');
    Route::get('payments/create/{relation_id?}/{invoice_id?}', 'PaymentController@create');
    Route::get('api/payments/{relation_id?}', ['as' => 'api.payments', 'uses' => 'PaymentController@getDatatable']);
    Route::post('payments/bulk', 'PaymentController@bulk');

    Route::resource('credits', 'CreditController');
    Route::get('credits/create/{relation_id?}/{invoice_id?}', 'CreditController@create');
    Route::get('api/credits/{relation_id?}', ['as' => 'api.credits', 'uses' => 'CreditController@getDatatable']);
    Route::post('credits/bulk', 'CreditController@bulk');

    Route::get('/resend_confirmation', 'LoginAccountController@resendConfirmation');
...
Route::group([
    ...
    Route::get('start_trial/{plan}', 'LoginAccountController@startTrial')
        ->where(['plan' => 'pro']);
    ...
    Route::get('settings/email_preview', 'LoginAccountController@previewEmail');
    Route::get('corporation/{section}/{subSection?}', 'LoginAccountController@redirectLegacy');
    ...
    Route::post('settings/change_plan', 'LoginAccountController@changePlan');
    Route::post('settings/cancel_company', 'LoginAccountController@cancelCompany');
    Route::post('settings/company_details', 'LoginAccountController@updateDetails');
    Route::get('settings/{section?}', 'LoginAccountController@showSection');
    Route::post('settings/{section?}', 'LoginAccountController@doSection');

    Route::post('user/setTheme', 'UserController@setTheme');
    Route::post('remove_logo', 'LoginAccountController@removeLogo');
    ...
    Route::get('bankrekeningen/import_ofx', 'BankRekeningController@showImportOFX');
    Route::post('bankrekeningen/import_ofx', 'BankRekeningController@doImportOFX');
    Route::resource('bankrekeningen', 'BankRekeningController');
    Route::get('api/bankrekeningen', ['as' => 'api.bankrekeningen', 'uses' => 'BankRekeningController@getDatatable']);
    Route::post('bankrekeningen/bulk', 'BankRekeningController@bulk');
    Route::post('bankrekeningen/validate', 'BankRekeningController@validateCompany');
    Route::post('bankrekeningen/import_expenses/{bank_id}', 'BankRekeningController@importExpenses');
});
...
Route::group(['middleware' => 'api', 'prefix' => 'api/v1'], function () {
    ...
    Route::get('login_accounts', 'CompanyApiController@getUserCompanies');
...
if (!defined('CONTACT_EMAIL')) {
    ...
    define('ENTITY_COMPANY_GATEWAY', 'account_gateway');
    ...
    define('ENTITY_PROSPECT', 'prospect');
    define('ENTITY_PROSPECT_ACTIVITY', 'prospect_activity');
    define('ENTITY_CUSTOMER', 'customer');
    define('ENTITY_CUSTOMER_ACTIVITY', 'customer_activity');
    ...
    define('ENTITY_BANK_REKENING', 'bankrekening');
    define('ENTITY_BANK_SUBREKENING', 'bank_subrekening');
    ...
    define('PERSON_PROSPECT_CONTACT', 'prospectcontact');
    define('PERSON_CUSTOMER_CONTACT', 'customercontact');
    ...
    define('COMPANY_BANKS', 'bankrekeningen');
    ...
    define('COMPANY_CHARTS_AND_REPORTS', 'charts_and_reports');
    ...
    define('SESSION_USER_COMPANIES', 'userCompanies');
    ...
    define('FEATURE_CUSTOMER_PORTAL_PASSWORD', 'relation_portal_password');
    define('FEATURE_CUSTOM_URL', 'custom_url');

    define('FEATURE_MORE_RELATIONS', 'more_relations'); // No trial allowed

    // Whitelabel
    define('FEATURE_CUSTOMER_PORTAL_CSS', 'relation_portal_css');
    ...
    define('WEPAY_RELATION_ID', env('WEPAY_RELATION_ID'));
    define('WEPAY_RELATION_SECRET', env('WEPAY_RELATION_SECRET'));
