...
class Utils
{
    ...
    public static function allowNewCompanies()
    {
    ...
    public static function formatMoney($value, $currencyId = false, $countryId = false, $showCode = false)
    {
        ...
        if (!$countryId && Auth::check()) {
            $countryId = Auth::user()->loginaccount->country_id;
        }
    ...
    public static function trackViewed($name, $type, $url = false)
    {
        ...
        $object = new stdClass();
        $object->companyId = Auth::user()->company_id;
        $object->url = $url;
        ...
        for ($i = 0; $i < count($viewed); $i++) {
            ...
            if (isset($counts[$item->companyId])) {
                $counts[$item->companyId]++;
            } else {
                $counts[$item->companyId] = 1;
            }
        }

        array_unshift($data, $object);

        if (isset($counts[Auth::user()->company_id]) && $counts[Auth::user()->company_id] > RECENTLY_VIEWED_LIMIT) {
            array_pop($data);
        }
    ...
    public static function setupWePay($accountGateway = null)
    {
        if (WePay::getEnvironment() == 'none') {
            if (WEPAY_ENVIRONMENT == WEPAY_STAGE) {
                WePay::useStaging(WEPAY_RELATION_ID, WEPAY_RELATION_SECRET);
            } else {
                WePay::useProduction(WEPAY_RELATION_ID, WEPAY_RELATION_SECRET);
            }
        }
