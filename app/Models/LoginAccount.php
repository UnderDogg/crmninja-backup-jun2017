 ...
 * Class Company
 */
class LoginAccount extends Eloquent
{
     ...
     * @var string
     */
    protected $presenter = 'App\Ninja\Presenters\UserAccountPresenter';
    ...
    public static $advancedSettings = [
        ...
        COMPANY_BANKS,
        COMPANY_CUSTOMER_PORTAL,
        COMPANY_CHARTS_AND_REPORTS,
     ...
     */
    public function useraccount_tokens()
    {
        return $this->hasMany('App\Models\UserAccountToken');
    }
    ...
    {
        return $this->hasMany('App\Models\BankRekening');
    }
    ...
    */
    ...
    protected function calculateLogoDetails()
    {
        ...
        if ($disk->exists($this->company_key . '.png')) {
            $this->logo = $this->company_key . '.png';
        } else if ($disk->exists($this->company_key . '.jpg')) {
            $this->logo = $this->company_key . '.jpg';
        }
    ...
    {
        foreach ($this->useraccount_tokens as $token) {
     ...
     * @return bool
     */
    public function isNinjaCompany()
    {
        return $this->company_key === NINJA_COMPANY_KEY;
    }
    ...
    public function hasFeature($feature)
    {
        ...
        if (!$selfHost && function_exists('ninja_company_features')) {
            $result = ninja_company_features($this, $feature);
        ...
        switch ($feature) {
            ...
            case FEATURE_API:
            case FEATURE_CUSTOMER_PORTAL_PASSWORD:
            ...
            case FEATURE_WHITE_LABEL:
                if ($this->isNinjaCompany() || (!$selfHost && $planDetails && !$planDetails['expires'])) {
                    return false;
                }
            // Fallthrough
            case FEATURE_CUSTOMER_PORTAL_CSS:
    ...
    {
        if (!$this->corporation->trial_plan) {
            if ($plan) {
        ...
        if ($this->corporation->trial_plan == PLAN_PRO) {
            if ($plan) {
    ...
    public function getDefaultEmailSubject($entityType)
    {
        ...
        return trans("texts.{$entityType}_subject", ['invoice' => '$invoice', 'loginaccount' => '$company']);
    }
    ...
    public function relationViewCSS()
    {
        ...
        if ($this->hasFeature(FEATURE_CUSTOMER_PORTAL_CSS)) {
