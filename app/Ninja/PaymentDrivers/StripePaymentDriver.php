...
class StripePaymentDriver extends BasePaymentDriver
{
    ...
    protected function paymentDetails($paymentMethod = false)
    {
        ...
        if ( ! empty($this->input['plaidPublicToken'])) {
            ...
            $data['plaidCompanyId'] = $this->input['plaidCompanyId'];
            unset($data['card']);
        }
    ...
    public function createToken()
    {
        ...
        if ( ! empty($data['plaidPublicToken'])) {
            $plaidResult = $this->getPlaidToken($data['plaidPublicToken'], $data['plaidCompanyId']);
            unset($data['plaidPublicToken']);
            unset($data['plaidCompanyId']);
            $data['token'] = $plaidResult['stripe_bank_company_token'];
        }
    ...
    protected function creatingPaymentMethod($paymentMethod)
    {
        ...
        if (!empty($data['object']) && ($data['object'] == 'card' || $data['object'] == 'bankrekening')) {
            $source = $data;
        ...
        // For older users the Stripe loginaccount may just have the customer token but not the card version
    ...
    {
        $relationId = $this->accountGateway->getPlaidRelationId();
        ...
        try {
            ...
            $response = (new \GuzzleHttp\Relation(['base_uri'=>"https://{$subdomain}.plaid.com"]))->request(
                'POST',
                'exchange_token',
                [
    ...
    public function makeStripeCall($method, $url, $body = null)
    {
        ...
        try{
            ...
            $response = (new \GuzzleHttp\Relation(['base_uri'=>'https://api.stripe.com/v1/']))->request(
                $method,
                $url,
                $options
            );
    ...
    public function handleWebHook($input)
    {
        ...
        $companyId = $accountGateway->company_id;
        ...
        if ($eventType == 'charge.failed' || $eventType == 'charge.succeeded' || $eventType == 'charge.refunded') {
        ...
        } elseif($eventType == 'customer.source.updated' || $eventType == 'customer.source.deleted' || $eventType == 'customer.bankrekening.deleted') {
            ...
            if ($eventType == 'customer.source.deleted' || $eventType == 'customer.bankrekening.deleted') {
