...
class CompanyRepository
{
    ...
    private function getCompanySearchData($user)
    {
        ...
        if ($company->custom_relation_label1) {
            $data[$company->custom_relation_label1] = [];
        }
        if ($company->custom_relation_label2) {
            $data[$company->custom_relation_label2] = [];
        }
    ...
    public function enablePlan($plan, $credit = 0)
    {
        ...
        $relation = $this->getNinjaRelation($company);
    ...
    public function createNinjaInvoice($relation, $relationCompany, $plan, $credit = 0)
    {
        $term = $plan['term'];
        ...
        $invoice->invoice_date = $relationCompany->getRenewalDate();
    ...
    public function getNinjaRelation($company)
    {
        $company->load('users');
        $ninjaCompany = $this->getNinjaCompany();
        $ninjaUser = $ninjaCompany->getPrimaryUser();
        $relation = Relation::whereCompanyId($ninjaCompany->id)
        ...
        if (!$relation) {
            ...
            $relation->company_id = $ninjaCompany->id;
    ...
    public function prepareUsersData($record)
    {
        ...
        foreach ($users as $user) {
            ...
            $item->company_id = $user->loginaccount->id;
            $item->company_name = $user->loginaccount->getDisplayName();
        ...
        if (!empty($primaryUser)) {
            foreach ($users as $user) {
                ...
                if ($user->loginaccount->corporation_id != $primaryUser->loginaccount->corporation_id) {
                    foreach ($user->loginaccount->corporation->companies as $company) {
                        $company->corporation_id = $primaryUser->loginaccount->corporation_id;
                        $company->save();
                    }
    ...
    public function unlinkUser($userCompanyId, $userId) {
        $userCompany = LoginAccount::whereId($userCompanyId)->first();
    ...
    public function getLoginAccountId($company)
    {
