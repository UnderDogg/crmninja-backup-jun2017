...
class InvoiceRepository extends BaseRepository
{
    ...
    public function getInvoices($companyId, $relationPublicId = false, $entityType = ENTITY_INVOICE, $filter = false)
    {
    ...
    public function getRecurringInvoices($companyId, $relationPublicId = false, $filter = false)
    {
        $query = DB::table('invoices')
                    ->join('companies', 'companies.id', '=', 'invoices.company_id')
                    ->join('relations', 'relations.id', '=', 'invoices.relation_id')
    ...
    public function getRelationRecurringDatatable($contactId)
    {
        $query = DB::table('invitations')
          ...
          ->select(
                ...
                'invoices.start_date',
                'invoices.end_date',
                'invoices.relation_enable_auto_bill',
                'frequencies.name as frequency'
            );
        ...
        $table = \Datatable::query($query)
            ...
            ->addColumn('relation_enable_auto_bill', function ($model) {
                if ($model->relation_enable_auto_bill) {
    ...
    public function getRelationDatatable($contactId, $entityType, $search)
    {
        $query = DB::table('invitations')
          ->join('companies', 'companies.id', '=', 'invitations.company_id')
          ->join('invoices', 'invoices.id', '=', 'invitations.invoice_id')
    ...
    public function save(array $data, Invoice $invoice = null)
    {
        ...
        if ($invoice) {
            ...
            $invoice = $company->createInvoice($entityType, $data['relation_id']);
        ...
        if ($invoice->is_recurring) {
            ...
            $invoice->relation_enable_auto_bill = isset($data['relation_enable_auto_bill']) && $data['relation_enable_auto_bill'] ? true : false;
        ...
        } elseif ($isNew && $company->{"{$entityType}_terms"}) {
        ...
        foreach ($data['invoice_items'] as $item) {
            ...
            if (isset($item['task_public_id']) && $item['task_public_id']) {
                ...
                if(Auth::user()->can('edit', $task)){
                    ...
                    $task->relation_id = $invoice->relation_id;
                    $task->save();
                }
            }
            ...
            if (isset($item['expense_public_id']) && $item['expense_public_id']) {
                ...
                if(Auth::user()->can('edit', $expense)){
                    ...
                    $expense->relation_id = $invoice->relation_id;
                    $expense->save();
                }
            }

            if ($productKey = trim($item['product_key'])) {
                if (\Auth::user()->loginaccount->update_products && ! strtotime($productKey) && ! $task && ! $expense) {
    ...
    public function cloneInvoice(Invoice $invoice, $quotePublicId = null)
    {
        ...
        $clone->invoice_number = $invoiceNumber ?: $company->getNextInvoiceNumber($clone);
    ...
    public function findInvoiceByInvitation($invitationKey)
    {
        ...
        $invoice->load('user', 'invoice_items', 'documents', 'invoice_design', 'loginaccount.country', 'relation.contacts', 'relation.country');
     ...
     * @return mixed
     */
    public function findOpenInvoices($relationId)
    {
        return Invoice::scope()
                ->invoiceType(INVOICE_TYPE_STANDARD)
                ->whereRelationId($relationId)
    ...
    {
        $recurInvoice->load('loginaccount.timezone', 'invoice_items', 'relation', 'user');

        if ($recurInvoice->relation->deleted_at) {
            return false;
        }
        ...
        $invoice->relation_id = $recurInvoice->relation_id;
        $invoice->recurring_invoice_id = $recurInvoice->id;
        $invoice->invoice_number = $recurInvoice->loginaccount->getNextInvoiceNumber($invoice);
        ...
        if ($recurInvoice->getAutoBillEnabled() && !$recurInvoice->loginaccount->auto_bill_on_due_date) {
