...
class UserAccountRepository
{
    ...
    {
        $data = $this->getUserAccountSearchData($user);
    ...
    private function getUserAccountSearchData($user)
    {
    ...
    private function getNavigationSearchData()
    {
        ...
        $settings = array_merge(UserAccount::$basicSettings, UserAccount::$advancedSettings);
    ...
    {
        $loginaccount = $this->getNinjaUserAccount();

        $lastCredit = Credit::withTrashed()->whereUserAccountId($loginaccount->id)->orderBy('public_id', 'DESC')->first();
    ...
    public function createNinjaInvoice($relation, $relationUserAccount, $plan, $credit = 0)
    {
        $term = $plan['term'];
        ...
        $loginaccount = $this->getNinjaUserAccount();
        $lastInvoice = Invoice::withTrashed()->whereUserAccountId($loginaccount->id)->orderBy('public_id', 'DESC')->first();
        ...
        $invoice->invoice_date = $relationUserAccount->getRenewalDate();
    ...
    public function getNinjaUserAccount()
    {
        $loginaccount = UserAccount::whereUserAccountKey(NINJA_COMPANY_KEY)->first();
    ...
    public function getNinjaRelation($loginaccount)
    {
        ...
        $ninjaUserAccount = $this->getNinjaUserAccount();
        $ninjaUser = $ninjaUserAccount->getPrimaryUser();
        $relation = Relation::whereUserAccountId($ninjaUserAccount->id)
        ...
        if (!$relation) {
            ...
            $relation->useraccount_id = $ninjaUserAccount->id;
        ...
        if ($relationExists) {
            ...
            $contact->useraccount_id = $ninjaUserAccount->id;
    ...
    {
        $loginaccount = UserAccount::whereUserAccountKey($key)
    ...
    public function unlinkUserAccount($loginaccount) {
    ...
    {
        return UserAccount::whereRaw('enable_reminder1 = 1 OR enable_reminder2 = 1 OR enable_reminder3 = 1')->get();
    }
    ...
    public function getUserUserAccountId($loginaccount)
    {
