...
class PushService
{
    ...
    public function sendNotification(Invoice $invoice, $type)
    {
        ...
        if (!$this->checkDeviceExists($invoice->loginaccount))
            return;

        //Harvest an array of devices that are registered for this notification type
        $devices = json_decode($invoice->loginaccount->devices, TRUE);
     ...
     *
     * Returns a boolean if this loginaccount has devices registered for PUSH notifications
     *
