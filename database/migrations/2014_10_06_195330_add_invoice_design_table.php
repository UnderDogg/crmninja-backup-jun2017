...
class AddInvoiceDesignTable extends Migration
{
    ...
    public function up()
    {
	    ...
	    var currencyId = customer.currency_id;
	    ...
	    displayCompany(doc, invoice, 220, layout.companyTop, layout);
	    ...
	    var customerHeight = displayRelation(doc, invoice, 220, 170, layout);
	    var detailsHeight = Math.max(invoiceHeight, customerHeight);
        ...
        DB::table('invoice_designs')->where('id', 2)->update([
            'javascript' => "  var GlobalY=0;//Y position of line at current page
			  ...
			  displayCompany(doc, invoice, 300, layout.companyTop, layout);


			  var y = layout.companyTop;
        ...
        var customerHeight = displayRelation(doc, invoice, layout.marginLeft, z, layout);
        layout.tableTop += Math.max(0, customerHeight - 75);
			  marginLeft2=395;
        ...
        DB::table('invoice_designs')->where('id', 3)->update([
            'javascript' => "    var customer = invoice.customer;
	    var loginaccount = invoice.loginaccount;
	    var currencyId = customer.currency_id;
	    ...
	    //second column
	    doc.setFontType('bold');
	    var name = invoice.loginaccount.name;    
	    if (name) {
	        doc.setFontSize('30');
	        doc.setFontType('bold');
	        doc.text(40, 50, name);
	    ...
	    doc.setFontSize(10);  
	    var marginLeft = 340;
	    displayCompany(doc, invoice, marginLeft, 780, layout);


	    SetPdfColor('White',doc);    
	    doc.setFontSize('8');
	    var detailsHeight = displayInvoice(doc, invoice, layout.headerRight, layout.companyTop-10, layout);
      ...
      layout.tableTop += 25;
		  ...
		  displayCompany(doc, invoice, layout.marginLeft, layout.companyTop, layout);
