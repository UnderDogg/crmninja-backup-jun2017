...
class AddSupportForBots extends Migration
{
    ...
    public function up()
    {
        Schema::create('security_codes', function ($table) {
            ...
            $table->foreign('user_id', 'fk_securitycode_user')->references('id')->on('users')->onDelete('cascade');
            $table->foreign('contact_id', 'fk_securitycode_contact')->references('id')->on('contacts')->onDelete('cascade');
            $table->foreign('company_id', 'fk_securitycode_company')->references('id')->on('companies')->onDelete('cascade');
        });
