...
NINJA.relationDetails = function(invoice) {
    ...
    var data;
    if (!relation || !relation.contacts.length) {
        return [];
    }
    var account = invoice.account;
    ...
    data = [
        ...
        {text:relation.address2},
        {text:cityStatePostal},
        {text:relation.country ? relation.country.name : ''},
        {text:relationEmail},
        {text: relation.custom_value1 && !custom1InPattern ? account.custom_relation_label1 + ' ' + relation.custom_value1 : false},
        {text: relation.custom_value2 && !custom2InPattern ? account.custom_relation_label2 + ' ' + relation.custom_value2 : false}
    ];
