...
@section('content')
    ...
    @include('companies.nav', ['selected' => COMPANY_PAYMENTS])
    ...
    @if ($accountGateway)
        ...
        {!! Former::populateField('plaid_relation_id', $accountGateway->getPlaidRelationId() ? str_repeat('*', strlen($accountGateway->getPlaidRelationId())) : '') !!}
            ...
            @if ($gateway->id == GATEWAY_STRIPE)
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-8 col-sm-8 help-block">
                        <input type="text"  class="form-control" onfocus="$(this).select()" readonly value="{{ URL::to(env('WEBHOOK_PREFIX','').'payment_hook/'.$company->company_key.'/'.GATEWAY_STRIPE) }}">
                        <div class="help-block"><strong>{!! trans('texts.stripe_webhook_help', [
                        'link'=>'<a href="https://dashboard.stripe.com/loginaccount/webhooks" target="_blank">'.trans('texts.stripe_webhook_help_link_text').'</a>'
                    ]) !!}</strong></div>
                    </div>
                </div>
