...
@section('content') 
    @parent
    @include('companies.nav', ['selected' => COMPANY_EMAIL_SETTINGS, 'advanced' => true])

    {!! Former::open()->rules([
            'iframe_url' => 'url'
        ])->addClass('warn-on-exit') !!}
    {{ Former::populate($company) }}
    {{ Former::populateField('pdf_email_attachment', intval($company->pdf_email_attachment)) }}
    {{ Former::populateField('document_email_attachment', intval($company->document_email_attachment)) }}
    {{ Former::populateField('enable_email_markup', intval($company->enable_email_markup)) }}
        ...
        <div class="panel-body form-padding-right">
            ...
            @if (Utils::isNinja())
                {!! Former::inline_radios('custom_invoice_link')
                        ...
                        ])->check($company->iframe_url ? 'website' : 'subdomain') !!}
