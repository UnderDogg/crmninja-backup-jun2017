...
@section('content')
	@parent
    @include('companies.nav', ['selected' => COMPANY_INVOICE_DESIGN, 'advanced' => true])
  ...
  <script>
    ...
    function getPDFString(cb) {
      ...
      invoice.loginaccount.hide_quantity = $('#hide_quantity').is(":checked");
      invoice.loginaccount.invoice_embed_documents = $('#invoice_embed_documents').is(":checked");
      invoice.loginaccount.hide_paid_to_date = $('#hide_paid_to_date').is(":checked");
      invoice.invoice_design_id = $('#invoice_design_id').val();
      invoice.loginaccount.page_size = $('#page_size option:selected').text();
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      {!! Former::populateField('invoice_design_id', $company->invoice_design_id) !!}
      {!! Former::populateField('body_font_id', $company->getBodyFontId()) !!}
      {!! Former::populateField('header_font_id', $company->getHeaderFontId()) !!}
      {!! Former::populateField('live_preview', intval($company->live_preview)) !!}
      {!! Former::populateField('font_size', $company->font_size) !!}
      {!! Former::populateField('page_size', $company->page_size) !!}
      {!! Former::populateField('invoice_embed_documents', intval($company->invoice_embed_documents)) !!}
      {!! Former::populateField('primary_color', $company->primary_color) !!}
      {!! Former::populateField('secondary_color', $company->secondary_color) !!}
      {!! Former::populateField('hide_quantity', intval($company->hide_quantity)) !!}
      {!! Former::populateField('hide_paid_to_date', intval($company->hide_paid_to_date)) !!}
      {!! Former::populateField('all_pages_header', intval($company->all_pages_header)) !!}
      {!! Former::populateField('all_pages_footer', intval($company->all_pages_footer)) !!}
            ...
            <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane" id="headerFooter">
                    ...
                    {!! Former::inline_radios('all_pages_header')
                            ...
                            ])->check($company->all_pages_header) !!}
                    ...
                    {!! Former::inline_radios('all_pages_footer')
                            ...
                            ])->check($company->all_pages_footer) !!}
    ...
    <br/>
      ...
      @include('invoices.pdf', ['company' => Auth::user()->loginaccount, 'pdfHeight' => 800])
