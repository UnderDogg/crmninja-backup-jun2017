...
@section('content')
    @parent
    @include('companies.nav', ['selected' => COMPANY_TEMPLATES_AND_REMINDERS, 'advanced' => true])
    ...
    @foreach ([REMINDER1, REMINDER2, REMINDER3] as $type)
        @foreach (['enable', 'num_days', 'direction', 'field'] as $field)
            {{ Former::populateField("{$field}_{$type}", $company->{"{$field}_{$type}"}) }}
        @endforeach
    @endforeach
            ...
            <div class="row">
                <div role="tabpanel">
                    ...
                    <div class="tab-content">
                        ...
                        @include('companies.template', ['field' => 'quote'])
                        @include('companies.template', ['field' => 'payment'])
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div role="tabpanel">
                    ...
                    <div class="tab-content">
                        ...
                        @include('companies.template', ['field' => 'reminder2', 'isReminder' => true])
                        @include('companies.template', ['field' => 'reminder3', 'isReminder' => true])
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="modal-body">
                    ...
                    <ul>
                        ...
                        @if ($company->custom_invoice_text_label2)
                            <li>$customInvoice1</li>
                        @endif
                        @if (count($company->account_gateways) > 1)
    ...
    <script type="text/javascript">
        ...
        function processVariables(str) {
            ...
            var passwordHtml = "{!! $company->isPro() && $company->enable_portal_password && $company->send_portal_password?'<p>'.trans('texts.password').': 6h2NWNdw6<p>':'' !!}";
