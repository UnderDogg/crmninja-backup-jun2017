...
@section('head')
    ...
    <script type="text/javascript">
        ...
        function handleSignedUp() {
            ...
            trackEvent('/loginaccount', '/signed_up');
        }
        ...
        function wordWrapText(value, width) {
                    @if (Auth::user()->loginaccount->auto_wrap)
            ...
            if (!window.loadedSearchData) {
                ...
                var request = $.get('{{ URL::route('get_search_data') }}', function (data) {
                    $('#search').typeahead({
                            ...
                            @if (Auth::check() && Auth::user()->loginaccount->custom_relation_label1)
                            , {
                                name: 'data',
                                limit: 3,
                                display: 'value',
                                source: searchData(data['{{ Auth::user()->loginaccount->custom_relation_label1 }}'], 'tokens'),
                                templates: {
                                    header: '&nbsp;<span style="font-weight:600;font-size:16px">{{ Auth::user()->loginaccount->custom_relation_label1 }}</span>'
                                }
                            }
                            @endif
                            @if (Auth::check() && Auth::user()->loginaccount->custom_relation_label2)
                            , {
                                name: 'data',
                                limit: 3,
                                display: 'value',
                                source: searchData(data['{{ Auth::user()->loginaccount->custom_relation_label2 }}'], 'tokens'),
                                templates: {
                                    header: '&nbsp;<span style="font-weight:600;font-size:16px">{{ Auth::user()->loginaccount->custom_relation_label2 }}</span>'
                                }
                            }
                            @endif
        ...
        $(function () {
            ...
            $('#signUpModal').on('shown.bs.modal', function () {
                trackEvent('/loginaccount', '/view_sign_up');
                $(['first_name', 'last_name', 'email', 'password']).each(function (i, field) {
                    var $input = $('form.signUpForm #new_' + field);
                    if (!$input.val()) {
                        $input.focus();
        ...
        <div class="container">
            @if (Utils::isNinjaProd())
                @if (Auth::check() && Auth::user()->isTrial())
                    {!! trans(Auth::user()->loginaccount->getCountTrialDaysLeft() == 0 ? 'texts.trial_footer_last_day' : 'texts.trial_footer', [
                            'count' => Auth::user()->loginaccount->getCountTrialDaysLeft(),
                            'link' => link_to('/settings/company_management?upgrade=true', trans('texts.click_here'))
                        ]) !!}
                @endif
            @else
                ...
                |
                @if (Auth::user()->loginaccount->hasFeature(FEATURE_WHITE_LABEL))
                    {{ trans('texts.white_labeled') }}
                @else
                    <a href="#"
                       onclick="loadImages('#whiteLabelModal');$('#whiteLabelModal').modal('show');">{{ trans('texts.white_label_link') }}</a>

                    <div class="modal fade" id="whiteLabelModal" tabindex="-1" role="dialog"
                         aria-labelledby="whiteLabelModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h4 class="modal-title"
                                        id="myModalLabel">{{ trans('texts.white_label_header') }}</h4>
                                </div>
                                ...
                                <div class="panel-body">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <img src="{{ BLANK_IMAGE }}"
                                                 data-src="{{ asset('images/pro_plan/white_label_before.png') }}"
                                                 width="100%" alt="before">
                                        </div>
                                        <div class="col-md-6">
                                            <h4>{{ trans('texts.after') }}</h4>
                                            <img src="{{ BLANK_IMAGE }}"
                                                 data-src="{{ asset('images/pro_plan/white_label_after.png') }}"
                                                 width="100%" alt="after">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="modal-footer" id="signUpFooter" style="margin-top: 0px">
                                    ...
                                    <button type="button" class="btn btn-primary"
                                            onclick="buyProduct('{{ WHITE_LABEL_AFFILIATE_KEY }}', '{{ PRODUCT_WHITE_LABEL }}')">{{ trans('texts.buy') }} </button>
                                </div>
                            </div>
                        </div>
                    </div>
