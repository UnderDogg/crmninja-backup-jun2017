<script type="text/javascript">
    ...
    function ViewModel(data) {
        ...
        self.invoice_taxes = ko.observable({{ Auth::user()->loginaccount->invoice_taxes ? 'true' : 'false' }});
        self.invoice_item_taxes = ko.observable({{ Auth::user()->loginaccount->invoice_item_taxes ? 'true' : 'false' }});
        self.show_item_taxes = ko.observable({{ Auth::user()->loginaccount->show_item_taxes ? 'true' : 'false' }});
    ...
    function InvoiceModel(data) {
        ...
        self.loginaccount = {!! $company !!};
        self.id = ko.observable('');
        self.discount = ko.observable('');
        self.is_amount_discount = ko.observable(0);
        self.frequency_id = ko.observable(4); // default to monthly
        self.terms = ko.observable('');
        self.default_terms = ko.observable(loginaccount.{{ $entityType }}_terms);
        self.terms_placeholder = ko.observable({{ !$invoice->id && $company->{"{$entityType}_terms"} ? "loginaccount.{$entityType}_terms" : false}});
        self.set_default_terms = ko.observable(false);
        self.invoice_footer = ko.observable('');
        self.default_footer = ko.observable(loginaccount.invoice_footer);
        self.footer_placeholder = ko.observable({{ !$invoice->id && $company->invoice_footer ? 'loginaccount.invoice_footer' : false}});
        ...
        self.relation_enable_auto_bill = ko.observable(false);
        ...
        self.formatMoney = function (amount) {
            ...
            return formatMoneyCompany(amount, self.loginaccount, relation);
        }
        ...
        self.totals.rawPaidToDate = ko.computed(function () {
            return roundToTwo(companying.toFixed(self.amount(), 2) - companying.toFixed(self.balance(), 2));
        });
        ...
        self.totals.rawTotal = ko.computed(function () {
            var total = companying.toFixed(self.totals.rawSubtotal(), 2);
            var discount = self.totals.rawDiscounted();
            total -= discount;
        ...
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            $element.typeahead({
                    ...
                    },
                    {
                        ...
                        @if (Auth::user()->loginaccount->fill_products)
                ...
                if (datum.cost) {
                    model.cost(companying.toFixed(datum.cost, 2));
                }
                if (!model.qty()) {
                    model.qty(1);
                }
