<iframe id="theFrame" style="display:block" frameborder="1" width="100%"
        height="{{ isset($pdfHeight) ? $pdfHeight : 1180 }}px"></iframe>
                ...
                <div class="modal-footer" id="signUpFooter">
                    ...
                    @if (Utils::isNinjaProd())
                        <a class="btn btn-primary"
                           href="{{ url('/settings/company_management?upgrade=true') }}">{{ trans('texts.go_pro') }}</a>
                    @else
...
<script type="text/javascript">
    ...
    @if ($company->hasLogo())
            window.companyLogo = "{{ Form::image_data($company->getLogoRaw(), true) }}";
    if (window.invoice) {
    ...
    function showMoreDesigns() {
        ...
        trackEvent('/loginaccount', '/view_more_designs');
