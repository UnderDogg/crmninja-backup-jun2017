...
@section('head')
    ...
    <style type="text/css">{!! isset($company)?$company->relationViewCSS():'' !!}</style>
    @stop
                ...
                <div class="navbar-header">
                    ...
                    @if (!isset($company) || !$company->hasFeature(FEATURE_WHITE_LABEL))
                        ...
                        <a class="navbar-brand" href="{{ URL::to(NINJA_WEB_URL) }}" target="_blank"><img
                                    src="{{ asset('images/invoiceninja-logo.png') }}" style="height:20px"></a>
                    @endif
                </div>
                <div id="navbar" class="collapse navbar-collapse">
                    @if (!isset($company) || $company->enable_relation_portal)
                        <ul class="nav navbar-nav navbar-right">
                            @if (isset($company) && $company->enable_relation_portal_dashboard)
                            ...
                            @if (isset($company) && $company->hasFeature(FEATURE_DOCUMENTS))
                                <li {{ Request::is('*relation/documents') ? 'class="active"' : '' }}>
                                    {!! link_to('/relation/documents', trans('texts.documents') ) !!}
                                </li>
                            @endif
                            @if (isset($company) && $company->getTokenGatewayId() && !$company->enable_relation_portal_dashboard)
